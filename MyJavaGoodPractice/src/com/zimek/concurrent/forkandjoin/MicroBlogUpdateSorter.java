package com.zimek.concurrent.forkandjoin;
...
public class MicroBlogUpdateSorter extends RecursiveAction {
	...
	protected void compute() {
		if (size() < SMALL_ENOUGH) {
			...
			int mid = size() / 2;
			MicroBlogUpdateSorter left = new MicroBlogUpdateSorter(updates,
					start, start + mid);
			MicroBlogUpdateSorter right = new MicroBlogUpdateSorter(updates,
					start + mid, end);
